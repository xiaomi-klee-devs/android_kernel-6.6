...
PACKAGE_KERNEL() {
    ...
    ZIP_NAME="${ZIP_PREFIX}-$1-${time}.zip"
    (
...
MAIN() {
    CHECK_CLANG || {
        ...
        CHECK_CLANG || DIE "Clang not found even after download — check archive layout"
    }

    BUILD_KERNEL
    CLEAN_OLD_ZIPS
    PACKAGE_KERNEL "$1" "$2" "$3"
    PRINT_SUMMARY
}

rm -rf compile.log "${OUT_DIR}"
MAIN "$1" "$2" "$3" | tee compile.log
